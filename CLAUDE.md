...
- `n_trials` must be divisible by **20** (2 × `n_bar_positions`; default 10). Minimum valid value: 20.
- `n_absent` (settings key) must be even; `n_trials - n_absent` must be even. Default: 14 (35% of 40).
...
- **condition 0**: rank 0 = `FULL_GREEN` (#00ab78), rank 2 = `FULL_ORANGE` (#d56f2c)
- **condition 1**: rank 0 = `FULL_ORANGE` (#d56f2c), rank 2 = `FULL_GREEN` (#00ab78)
...
The target and all non-distractor items are always `GREY` (#8f8f8f). Points multipliers: rank 0 → ×1 (max 10 pts), rank 2 → ×100 (max 1000 pts). Absent trials also use rank 0 or 2 (reward non-predictive — no distractor shown).
